    ...
    case 'keycloak':
      strategy = new AuthStrategy(_.defaults({
        ...
        function(req, identifier, profile, done) {
          let data = jwt.decode(req);
          profile.roles = data.resource_access[options.clientID] ? data.resource_access[options.clientID].roles : [];
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(
                req.user.id, provider, authScheme, profile,
        ...
        req.logIn(user, function(err) {
          ...
          if (info && info.accessToken) {
            if (!!options.json) {
              ...
              res.cookie('sonicjs_access_token', info.accessToken.id.toString(), {
                signed: true,
                maxAge: 1000 * info.accessToken.ttl,
                domain: (options.domain) ? options.domain : null,
              });
